...
const pricingPlans = [
  {
    ...
    features: ['Custom Design', 'Responsive Build', 'Basic SEO', 'Content Integration'],
  },
  {
    id: '02',
    name: 'Growth',
    ...
    features: ['CMS Integration', 'Advanced Analytics', 'Newsletter', 'Priority Support'],
  },
  {
    id: '03',
    name: 'Commerce',
    ...
    features: ['Product Management', 'Payments', 'Inventory', 'Customer Accounts'],
  },
];

const Pricing = () => {
  return (
    <div id="pricing" className="bg-white text-min-black border-b border-min-black">
      {/* Header */}
      <div className="border-b border-min-black px-6 sm:px-8 lg:px-12 py-12 flex justify-between items-end">
        ...
        <span className="font-mono text-sm tracking-widest hidden md:block">ROI/MAX</span>
      </div>
          ...
          >
            <div className="p-8 border-b border-min-black group-hover:border-white/20 transition-colors">
              <div className="flex justify-between items-start mb-8">
                <h3 className="font-sans text-3xl font-medium uppercase tracking-tight">{plan.name}</h3>
                <span className="font-mono text-xs border border-current rounded-full px-2 py-1">{plan.id}</span>
              </div>
              <div className="text-5xl font-light tracking-tight mb-2">{plan.price}</div>
              <p className="font-mono text-xs uppercase tracking-widest opacity-70">{plan.description}</p>
            </div>

            <div className="flex-1 p-8">
              <ul className="space-y-4">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center text-sm font-light">
